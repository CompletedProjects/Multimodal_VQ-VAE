...
def make_vae(): # Nawid - Makes the encoder and the decoder
    encoders = [QuarterEncoder(3,1, 128, 512), HalfEncoder(128, 128, 512)] # Nawid -For the first encoder, the number of input channels is 3 for RGB images, 1 for grayscale and 4 for concatenation of RGB and Grayscale images and it produces 128 output. The 512 is the number of latents in the dictionary
    decoders = [HalfDecoder(128, 128), HalfQuarterDecoder(128, 3,1)]
...
class TopPrior(nn.Module): # Nawid -  As described in the paper, the top prior is equipped with a attention layers so it can benefit from a larger receptive field to capture correlations in spatial locations that are far apart in the image
    def __init__(self, depth=128, num_heads=2): # Nawid - 2 attention heads
        super().__init__()
        self.embed = nn.Embedding(512, depth) # Nawid - Makes the embedding dictionary where the depth is the dimensionality
        ...
        ) # Nawid - Obtains the output which as 512 output channels which I believe is related to the number of entries in the codebook/latent space ( which is related to the number of classes of indices which is used for the softmax)

    def forward(self, x):
        x = self.embed(x) # Nawid - Turns x (which are the indices from an encoder) into an embedding
        ...
        return self.out_stack(torch.cat([out1, out2], dim=1)) # Nawid- Concatenates the different outputs along dimension 1 (which I believe is the number of channels in pytorch convention) and then passes it through the self.out_stack which will give the logits
...
class BottomPrior(nn.Module): # Nawid - Bottom prior is used to encode local information and so operates at a larger resolution. Using an attention network fo this layer would also not be practical due to memory constraints.
    def __init__(self, depth=128, num_heads=2):
        super().__init__()
        self.embed_top = nn.Embedding(512, depth) # Nawid- Makes the embedding dictionary with the number of entries is 512 and the dimensionality is 128
        ...
        self.out_stack = nn.Sequential(
            nn.Conv2d(depth * 2, depth, 1),
            nn.Conv2d(depth, 512, 1),
        ) # Nawid - The dimensionality of the output is equal to the number of entries in the dictionary which is related to the number of classes in the loss function
    ...
    def forward(self, bottom, top):
        ...
        conds = self.cond_stack(conds) # Nawid-  Input the embeddings of the top layer into the conditional stack - so the bottom prior will likely be conditioned on information related to the embeddings of the top prior

        out = self.embed_bottom(bottom) # Nawid  - Changes the indices of the bottom layer into an embedding 
        out = out.permute(0, 3, 1, 2).contiguous()
        out1, out2 = self.pixel_cnn(out, conds=conds) # Nawid - Obtains the horizontal and vertical stacks from the pixel CNN which uses the bottom embeddings and conditional top embeddings
        return self.out_stack(torch.cat([out1, out2], dim=1)) # Nawid - Stack the outputs along the first dimension which is the channel dimension
