...
def main():
    ...
    for encoder in vae.encoders:
        encoded = encoder.encode(inputs) # Nawid- each encoder performs the encode method which downsamples the feature map
        inputs, _, latents = encoder.vq(encoded) # Nawid - Quantises the continious embedding feature map and gives the output embedding as well as the indices (latents)
        all_latents.append(latents) # 
    all_latents = all_latents[1:][::-1] # Nawid - Puts the second dimensions in reverse and gets all the entries except the first one -so it puts the indices of the different feature maps in reverse
    ...
    for i in range(context_len):
        ...
        latents[:, i] = [sample_softmax(x) for x in probs.detach().cpu().numpy()[:, :, i]] # Nawid- Sampling different index alues
