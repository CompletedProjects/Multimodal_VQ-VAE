...
def main():
    ...
    for row in range(results.shape[1]):
        for col in range(results.shape[2]):
            ...
            with torch.no_grad():
                outputs = torch.softmax(top_prior(partial_in), dim=1).cpu().numpy() # Nawid - Place input into the top_prior and then it performs softmax on the possible output
    ...
    with torch.no_grad():
        ...
        top_embedded = vae.encoders[1].vq.embed(full_latents) # Nawid - Changes the indices into embeddings to get the top feature map
