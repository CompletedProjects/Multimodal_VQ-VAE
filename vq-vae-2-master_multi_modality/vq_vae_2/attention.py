...
class PixelAttention(nn.Module):
    ...
    def forward(self, *images, conds=None):
        ...
        for image in images:
            ...
            result = image.permute(0, 2, 3, 1) # Nawid - Change the order
            result = result.view(batch, height * width, num_channels)
            result = self.attention(result) # Nawid - Perform attention mechanism
            result = result.view(batch, height, width, num_channels)
...
class MaskedAttention(nn.Module):
    ...
    def __init__(self, num_channels, num_heads=8):
        ...
        self.kqv_projection = nn.Linear(num_channels, num_channels * 3) # Nawid- Triple the number of channels to get k, q and v matrices
    ...
    def forward(self, sequence):
        ...
        keys, queries, values = [self._split_heads(x) for x in kqv] # Nawid - Obtain the k,q and v vectors
        logits = torch.bmm(queries, keys.permute(0, 2, 1)) 
        logits /= math.sqrt(self.num_channels / self.num_heads) # Nawid -Division term to prevent unstable gradients
        logits += self._logit_mask(sequence.shape[1])
        weights = F.softmax(logits, dim=-1) # Nawid- Perform softmax 
        weighted_sum = torch.bmm(weights, values) # Nawid - Weighted sum of softmax with the values
        combined = self._combine_heads(weighted_sum) # Nawid -  Combine the outputs of the different heads
        return self.mix_heads(combined) # Nawid - Linear layer after the combined head output

    def _split_heads(self, batch): # Nawid - Split up the channels into groups 
        """
    ...
    def _logit_mask(self, num_steps):
        ...
        upper = (row_indices >= col_indices) # Nawid - When row indices are greater than column indices I believe it is the upper triangular part
        mask = np.where(upper, 0, -np.inf).astype(np.float32) # Nawid - I believe this makes the logit to negative infinity so it softmax becomes 0
