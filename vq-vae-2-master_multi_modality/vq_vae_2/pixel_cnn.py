...
class PixelCNN(nn.Module):
    ...
    def forward(self, images, conds=None):
        ...
        rest take two.
				# Nawid - PixelConvA means that mask A is used whilst pixelconvB means a B mask is used
        Returns:
            A tuple (vertical, horizontal), one for each
              of the two directional stacks.
        """
        outputs = self.layers[0](images, conds=conds) # Nawid - First input layer
        for layer in self.layers[1:]: # Nawid - Second to end layers
...
class PixelConv(nn.Module):
    ...
    def __init__(self, depth_in, depth_out, cond_depth=None, horizontal=2, vertical=2):
        ...
        self.horizontal = horizontal # Nawid - Horizontal receptive field
        self.vertical = vertical # # Nawid - Vertical receptive field

        self._init_directional_convs()
        self.vert_to_horiz = nn.Conv2d(depth_out * 2, depth_out * 2, 1) # Nawi d- 1 x 1 convolutions to make the vertical join with the horizontal
    ...
    def _run_stacks(self, vert_in, horiz_in, conds):
        ...
        if conds is not None:
            ...
            vert_out = vert_out + cond_bias[:, :self.depth_out*2] # Nawid - Takes into account the conditonal bias
            horiz_out = horiz_out + cond_bias[:, self.depth_out*2:] # Nawid - Takes into account the conditonal bias

        vert_out = gated_activation(vert_out) # Nawid - Gated activation for the vertical stack
        horiz_out = gated_activation(horiz_out) # Nawid - Gated activation for the horizonal stack
...
class PixelConvA(PixelConv):
    ...
    def _init_directional_convs(self): # Nawid - Initial convolutions for the first layer ( when there is mask A)
    ...
    def _run_padded_vertical(self, vert_in):
        ...
        return self.vertical_conv(F.pad(vert_in, vert_pad))[:, :, :-1, :] # Nawid- Perform the convolutions

    def _run_padded_horizontal(self, horiz_in):
        return self.horizontal_conv(F.pad(horiz_in, (self.horizontal, 0, 0, 0)))[:, :, :, :-1] # Nawid- Perform the horizontal convolutions
...
class PixelConvB(PixelConv):
    ...
    def __init__(self, depth_in, cond_depth=None, norm=False, horizontal=2, vertical=2):
        super().__init__(depth_in, depth_in, cond_depth=cond_depth, horizontal=horizontal,
                         vertical=vertical)
        self.horiz_residual = nn.Conv2d(depth_in, depth_in, 1) # Nawid - convolution to make the output of the horizontal stack able to add as a residual
        self.vert_norm = lambda x: x
        self.horiz_norm = lambda x: x
        if norm:
            self.vert_norm = ChannelNorm(depth_in) # Nawid - Performs layer normalisation
    ...
    def forward(self, vert_in, horiz_in, conds=None):
        ...
        Returns:
            A tuple (vertical, horizontal), one for each
              of the two directional stacks.
        """
        vert_out, horiz_out = self._run_stacks(vert_in, horiz_in, conds) # Nawid- Performs the horizontal and vertical convolution
        horiz_out = horiz_in + self.horiz_norm(self.horiz_residual(horiz_out))  # Nawid - Horizontal stack output + residual ( where the residual is the output which is added to the input)
...
def gated_activation(outputs): # Nawid - Gated activation unit
